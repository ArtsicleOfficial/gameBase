...
class Assets {
    ...
    public static function init() {
        ...
        // build spirte altas directly from Aseprite file
        ...
        // LDtk file hot-reload
        #if debug
        var res = try hxd.Res.load(worldData.projectFilepath.substr(4)) catch(_) null; // assume the LDtk file is in the "res/" subfolder
        if( res != null )
            res.watch( ()->{
                // Only reload actual updated file from disk after a short delay, to avoid reading a file being written
    ...
    /**
        Pass `tmod` value from the game to atlses, to allow them to play animations at the same speed as the Game.
        For example, if the game has some slow-mo running, all atlas animations should also play in slow-mo.
    **/
